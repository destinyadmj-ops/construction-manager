...
for (let i=0;i<s.length;i++){ const ch=s[i]; const next=s[i+1]; if (ch==='\n'){ line++; inLineComment=false; }
 ...
 if (brace>maxBrace){ maxBrace=brace; maxLine=line; }
 if (ch==='<') angle++; if (ch==='>') { if (angle>0) angle--; }
}
console.log('brace balance', brace, 'angle balance', angle, 'maxBrace', maxBrace, 'maxLine', maxLine);
