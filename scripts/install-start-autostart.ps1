...
function Install-WithStartupShortcut {
  $startupInstaller = Join-Path $PSScriptRoot 'install-start-startup.ps1'
  if (-not (Test-Path $startupInstaller)) {
    throw "install-start-startup.ps1 not found: $startupInstaller"
  }
...
Write-Host 'To remove: powershell -NoProfile -ExecutionPolicy Bypass -File scripts/uninstall-start-autostart.ps1'